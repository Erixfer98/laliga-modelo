"""
generar_bbdd_laliga.py — descarga football-data.co.uk y regenera datos/bbdd_laliga.xlsx + .csv
(28 columnas, hojas partidos + diccionario). Cada corrida borra y reescribe todo.
Uso: python generar_bbdd_laliga.py
"""
...
URL = "https://www.football-data.co.uk/mmz4281/{codigo}/SP1.csv"
...
COLUMNAS = [
    "temporada_txt", "jornada_val", "fecha_dt", "hora_local_es_txt", "hora_utc_txt",
...
def descargar(codigo: str) -> pd.DataFrame:
    r = requests.get(URL.format(codigo=codigo), timeout=30)
    r.raise_for_status()
...
def a_horas(fecha: str, hora_uk: str):
    ...
    es = uk.astimezone(ZoneInfo("Europe/Madrid")).strftime("%H:%M")
...
def transformar(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["hora_local_es_txt"] = [h[0] for h in horas]
    ...
    out = pd.DataFrame({
        ...
        "hora_local_es_txt": df["hora_local_es_txt"],
...
def fila_partido(i, p):  # i = numero de fila en Excel
    return [
        p["temporada_txt"], int(p["jornada_val"]), p["fecha_dt"], p["hora_local_es_txt"], p["hora_utc_txt"],
...
def diccionario(partidos: pd.DataFrame, fecha_carga: str):
    ...
    return [
        ...
        ["temporada_txt", "texto", 'Temporada de La Liga en formato "2025-26" (ago-may).', "Codigo football-data.co.uk (2526, 2627)."],
        ...
        ["hora_local_es_txt", "texto", "Hora de inicio en Espana peninsular (Europe/Madrid), formato HH:MM. Vacia = no publicada.",
         "Columna Time de football-data.co.uk viene en hora de Reino Unido; se convierte a Espana (+1 h, ambos cambian horario de verano el mismo dia)."],
        ...
        ["COBERTURA", None, f"{len(partidos)} partidos ({cobertura}). 28 columnas. Cargado el {fecha_carga}.", "Descarga automatica con generar_bbdd_laliga.py."],
        ...
        ["COMO ACTUALIZAR", None, "Volver a correr generar_bbdd_laliga.py: descarga los CSV de las temporadas configuradas y regenera el Excel completo.",
...
if __name__ == "__main__":
    ...
    crudo = pd.concat([descargar(c) for c in TEMPORADAS], ignore_index=True)
    generar(transformar(crudo))
